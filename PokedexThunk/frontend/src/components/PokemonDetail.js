...
import { fetchDetails } from '../store/pokemon';
...
const PokemonDetail = () => {
  ...
  useEffect(() => {
    ...
    setEditItemId(null);
    dispatch(fetchDetails(pokemonId))
  }, [pokemonId]);
  ...
  } else if (showCreateItemForm && pokemon.captured) {
    content = (
    <CreateItemForm
    pokemon={pokemon}
    hideForm={() => setShowCreateItemForm(false)}
    />
    );
  } else {
    content = (
      <div className="pokemon-detail-lists">
        ...
        <div>
          <h2>
            Items 
            <button onClick={() => setShowCreateItemForm(true)}> + </button>
          </h2>
          <table>
            <thead>
              <tr>
