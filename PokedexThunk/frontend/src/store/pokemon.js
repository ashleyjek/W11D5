...
export const fetchDetails = (pokemonId) => async (dispatch) => {
...
export const createPokemon = (pokemon) => async (dispatch) => {
  const res = await PokemonApiUtil.postPokemon(pokemon);
  const data = await res.json();
  // debugger
  dispatch(addOnePokemon(data));
}
...
export const editPokemon = (pokemon) => async (dispatch) => {
  const res = await PokemonApiUtil.updatePokemon(pokemon);
  const data = await res.json();
  dispatch(addOnePokemon(data));
}
